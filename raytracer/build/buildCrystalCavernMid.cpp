...
void World::build(void) {
    ...
    vplane.top_left.x = -32;
    vplane.top_left.y = 53;
    vplane.top_left.z = 10;
    vplane.bottom_right.x = 32;
    vplane.bottom_right.y = 17;
    ...
    p2->shadows = false; add_light(p2);

    Directional *d = new Directional(Vector3D(0.15, -1, -0.3), RGBColor(1, 0.95f, 0.9f), 1.6f);
    d->shadows = false; add_light(d);

    PointLight *cf = new PointLight(Point3D(0, 50, 90), RGBColor(0.85f, 0.82f, 0.78f), 2.5f);
    cf->shadows = false; add_light(cf);
}
